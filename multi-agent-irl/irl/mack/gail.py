    ...
    def __init__(self, policy, ob_space, ac_space, nenvs, total_timesteps, nprocs=2, nsteps=200,
        ...
        def train(obs, states, rewards, masks, actions, values):
            ...
            for k in range(num_agents):
                ...
                print("pi")
                print(sess.run(train_model[k].pi[0:30], feed_dict=new_map))
            ...
            for k in range(model.num_agents):
                ...
                if update > 10:
                    ...
                    try:
                        ...
                        logger.record_tabular('reward %d' % k, float(np.sum(mh_rewards[k])))
                        logger.record_tabular('true reward %d' % k, float(np.sum(mh_true_rewards[k])))
